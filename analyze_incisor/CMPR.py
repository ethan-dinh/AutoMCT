...
This module provides functionality for performing curved multi-planar
reconstruction on incisor volumes. It extracts the centerline of the
incisor, computes local coordinate frames along the centerline, and
resamples the volume to create curved reformatted slices.
...
def load_volume(path: str) -> np.ndarray:
    ...
    """
    logging.info("Loading volume from %s", path)
...
def save_ct_volume_as_tiff(volume: np.ndarray, output_dir: str, name: str):
    ...
    logging.info(
        "Saving volume with shape: %s, dtype: %s, min: %s, max: %s",
    ...
    if volume.dtype == np.float64:
        logging.info("Downgrading float64 to float32")
...
def compute_bounding_box(volume: np.ndarray) -> tuple[slice, slice, slice]:
    ...
    return (
        min_coords,
        max_coords,
        tuple(slice(minc, maxc) for minc, maxc in zip(min_coords, max_coords)),
    )
...
def smooth_centerline(
    centerline: np.ndarray, smoothing_factor=5.0, n_points=100
) -> np.ndarray:
    """
    ...
    tck, _, _, _ = splprep(points, s=smoothing_factor)
    ...
    smoothed_centerline = np.stack(
        [smoothed_points[2], smoothed_points[1], smoothed_points[0]], axis=1
    )
...
def compute_centerline(mask: np.ndarray) -> np.ndarray:
    ...
    smoothed_centerline = smooth_centerline(
        centerline, smoothing_factor=smoothing_factor, n_points=n_points
    )
...
def cmpr_pipeline(path, output_dir, slice_size=64, spacing=0.5):
    ...
    logging.info("Cropping volume to bounding box")
    binary_mask = (volume > 0).astype(np.uint8)
    binary_mask = mca.segmentation.morphological_closing(
        binary_mask, ball_radius=2)
    ...
    logging.info(
        "Perform a morphological closing to fill holes in the binary mask")
    ...
    for i in tqdm(
        ...
        if (i + 1) % 10 == 0:
            logging.debug(
                "Generated %d/%d slices",
                i + 1,
                len(smoothed_centerline))

    slices_volume = np.stack(slices, axis=0)
    save_ct_volume_as_tiff(slices_volume, output_dir, "resliced_volume")
    logging.info("Saved slices to %s", output_dir)

    if IS_VISUALIZE:
        mca.visualization.create_3d_visualization(
            cropped_volume,
            None,
            {
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Curved Multi-Planar Reconstruction for Incisor",
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Slice width/height")
    parser.add_argument(
        "--spacing", type=float, default=1.0, help="Spacing between pixels"
    )
    parser.add_argument(
        "--visualize",
        "-v",
        action="store_true",
        help="Enable interactive 3D visualization",
    )
...
def main():
    ...
    # Find all of the folders in the input path (each folder is a stack of
    # files)
